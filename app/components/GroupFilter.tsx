...
import { countFixturesByGroup, listFixtureGroups } from "@/lib/fixture-groups";
...
export function GroupFilter({ fixtures, value, onChange }: Props) {
  ...
  const counts = useMemo(() => countFixturesByGroup(fixtures), [fixtures]);
  ...
  return (
    <div className="flex items-center gap-2">
      <label
        ...
        className="hidden text-[11px] font-semibold uppercase tracking-wide text-slate-200 sm:inline"
      >
        Group
      </label>
      <select
      ...
      >
        <option value="all">All groups ({fixtures.length})</option>
        {groups.map((group) => (
